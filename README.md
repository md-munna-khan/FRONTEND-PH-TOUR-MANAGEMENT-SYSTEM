...
- using zod schema in RegisterForm
```ts
import {
  Form,
  FormControl,
...
- password hide and show collect from originUi input fields
```ts
...
```
- add zod refine  from zod and implement all codes
```ts
import {
  Form,
  FormControl,
...
- when user register or login complete then we are check he is authentic user or not ? 
and we are verify check with otp system
- index.tsx
```ts
import App from "@/App";
